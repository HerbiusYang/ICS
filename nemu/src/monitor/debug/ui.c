...
static int cmd_d(char *args){
	...
	printf("please input correct number");
	...
	else if(sscanf(arg,"%3s",all)==1&&strcmp(all,"all")==1){
		init_wp_list();
		printf("Delete Compeletely...\n");
	}
	else{
		WP* status_H = head;
		WP* label = NULL;
		...
		if (label == NULL) {
		printf("Do not have NO.%d in watchpointpool\n",NO);
		}
	}
	}
	return 0;
...
static int cmd_w(char *args){
	...
	if (arg == NULL) {
		...
		printf("Have set the watchpoint at:NO.%d; %s= %u\n",wp->NO,arg,result);
	}
	return 0;
}
static struct {
